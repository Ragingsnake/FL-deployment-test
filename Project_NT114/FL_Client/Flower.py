...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # Build full state dict from incoming parameters (state_dict order),
        # then extract only trainable params by name for FedProx.
        # Direct zip(named_parameters(), parameters) is WRONG — state_dict has
        # 3 extra BN buffer tensors per BN layer (running_mean, running_var,
        # num_batches_tracked), shifting all keys after the first BN out of sync.
        full_state = {
            k: torch.tensor(v).to(DEVICE)
            for k, v in zip(self.model.state_dict().keys(), parameters)
        }
