...
try {
    ...
    if (-not $condaCmd) {
        # Try common conda locations under provided CondaRoot
        $candidates = @(
            (Join-Path $CondaRoot 'Scripts\conda.exe'),
            (Join-Path $CondaRoot 'condabin\conda.bat'),
            (Join-Path $CondaRoot 'Scripts\conda')
        )
        foreach ($cand in $candidates) {
            if (Test-Path $cand) {
                $condaCmd = Get-Command $cand -ErrorAction SilentlyContinue
                if ($condaCmd) { break }
            }
        }
    }
    ...
    if ($condaCmd) {
        $condaPath = $condaCmd.Path
        Write-Output "Using conda at $condaPath -- running Python in 'gymenv' via 'conda run'"
        & $condaPath run -n gymenv python $tmp $workdir.Path
    }
    else {
        $pyCmd = Get-Command python -ErrorAction SilentlyContinue
        if ($pyCmd) {
            Write-Output "'conda' not found; running system Python"
            & python $tmp $workdir.Path
        }
        else {
            Write-Error "Neither 'conda' nor 'python' found. Provide a valid CondaRoot or ensure Python is available."
            exit 1
        }
    }
} finally {
...
Write-Output "apply-fixes completed for $($workdir.Path)"
